$(function () {
    ...
    $.fn.Page = function (options) {
        ...
        //获取当前页面的路径
        opts.a_href = window.location.pathname;
        ...
        function addContent() {
            ...
            var liStr = "";
            //首页链接
            var first_href = opts.a_href + "?page=1";
            //尾页链接
            var last_href = opts.a_href + "?page=" + opts.totalPages;
            //前一页链接
            var prv_href = "";
            var next_href = "";
            if (opts.activePage == 1) {
                prv_href = opts.a_href + "?page=1";
            } else {
                prv_href = opts.a_href + "?page=" + (opts.active_page - 1);
            }
            //后一页链接
            if (opts.activePage == opts.totalPages) {
                next_href = opts.a_href + "?page=" + opts.totalPages;
            } else{
                next_href = opts.a_href + "?page=" + (opts.activePage + 1);
            }

            //填充列表内容
            for (var i = 1; i <= opts.liNums; i++) {
                liStr += "<li><a href=" + opts.a_href + "?page=" + i + ">" + i + "</a></li>"
            }
            //判断是否有相应的可选参数，如果有，添加对应的内容（从前到后）
            //前一页
            if (opts.hasPrv) {
                str += '<a href="' + prv_href + '" class="prv_a"><div class="prv fl">' + opts.prv + '</div></a>';
            }
            //首页
            if (opts.hasFirstPage) {
                str += '<a href="' + first_href + '" class="first_a"><div class="first fl">' + opts.firstPage + '</div></a>';
            }
            //列表
            str += '<ul class="pagingUl">' + liStr + '</ul>';
            //尾页
            if (opts.hasLastPage) {
                str += '<a href="' + last_href + '" class="last_a"><div class="last fl">' + opts.lastPage + '</div></a>';
            }
            //后一页
            if (opts.hasNext) {
                str += '<a href="' + next_href + '" class="next_a"><div class="next fl">' + opts.next + '</div></a>';
            }
        ...
        function addListener() {
            ...
            obj.on("click", ".prv", function () {
                ...
                //重绘
                var link = opts.a_href + "?page=" + prv_page;
                repaint(prv_page);
                $(".prv_a").attr("href", link);
            });
            //首页
            obj.on("click", ".first", function () {
                //重绘首页，首页的页数为1
                repaint(1);
                var link = opts.a_href + "?page=1";
                $(".first_a").attr("href", link);
            });
            ...
            obj.on("click", ".last", function () {
                ...
                var link = opts.a_href + "?page=" + opts.totalPages;
                $(".last_a").attr("href", link);
            });
            //下一页
            obj.on("click", ".next", function () {
                ...
                repaint(next_page);
                var link = opts.a_href + "?page=" + next_page;
                $(".next_a").attr("href", link);
            });
            //列表
        ...
        function repaint(index) {
            ...
            //获取当前列表的长度
            var totalNum = $("#page li a").length;
            //获取列表第一页的页数（不一定是首页）
            var first_page = parseInt($("#page li a").eq(0).html());
            //获取列表最后一页的页数（不一定是尾页）
        ...
        function movePage(first, last, active_page) {
            ...
            for (var i = first; i <= last; i++) {
                str += "<li><a href=" + opts.a_href + "?page=" + i + ">" + i + "</a></li>"
            }
        ...
        function addActiveClass(index) {
            ...
            for (var i = 0; i <= length; i++) {
                var li_number = parseInt($("#page li a").eq(i).html());
                if (index == li_number) {
                    $("#page li a").eq(i).addClass(opts.activeClass);
                }
            }
        }
